...
export class SimulationPipelineEffects {
    ...
    startSimulation$ = createEffect(() => {
        return this.actions$.pipe(
            ...
            switchMap(action =>
                this.simulationService.startSimulation({ body: action.request }).pipe(
                    tap(() =>
                        this.notificationService.info(
                            `Started simulation pipeline with conf: ${action.request.configurationName}`
                        )
                    ),
    ...
    getSimulationStepInfo$ = createEffect(() => {
        return this.actions$.pipe(
            ...
            switchMap(({ stepId }) =>
                this.statusService.getSimulationStepInfo({ id: stepId }).pipe(
                    concatMap(stepInfo => {
