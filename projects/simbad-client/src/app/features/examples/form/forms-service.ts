...
export class FormsService {
    ...
    private static generateIntValidators(
        parameter: ParameterDefinition
    ): ValidatorFn[] {
        return [
            Validators.min(parameter.maxValue),
            Validators.max(parameter.maxValue)
        ];
    }

    private static generateFloatValidators(
        parameter: ParameterDefinition
    ): ValidatorFn[] {
        return [
            Validators.min(parameter.maxValue),
        ...
        switch (parameter.valueType) {
            case 'int':
                validators = validators.concat(
                    FormsService.generateIntValidators(parameter)
                );
                break;
            case 'float':
                validators = validators.concat(
                    FormsService.generateFloatValidators(parameter)
                );
                break;
        }
        return validators;
    ...
    public addParameterControlToForm(
        form: FormGroup,
        ...
        if (node.definition.possibleClasses) {
            form = this.buildFormForNode(
                ...
                this.ods.toParameterTreeNode(
                    this.ods.getByClassName(node.definition.possibleClasses[0]),
                    node.path
                )
            );
        }
    ...
    public formValueToTree(value: any): any {
        ...
        Object.keys(value).forEach(key =>
            this.deepAssign(tree, key, value[key])
        );
        return tree;
    }
